 ...
 *             => 서버 닫기, 오라클 닫기, 파일 닫기... 이런경우에 많이 사용됨
 *          }
 *          
 ...
 *       ** try절이정상 수행을 하면 catch절은 수행하지 않는다
 ...
 *          -------
 *       }
 *       => catch는 위에서 한개씩 검색한후에 해당 예외처리 클래스를 찾아 준다
 *                 ----------------- 다중조건문과 동일(catch는 한개만 수행)
 *       = 간접처리(예외회피) : java 나 오픈라이브러리
 *         예외에 대해 예측되는 예외 클래스를 선언만 한다
 *         => 다른 메소드 에서 호출시에 처리 후 사용이 가능(try~catch)
 *         => 예외클래스에 순서가 존재하지 않는다
 ...
 *            => 예외처리를 반드시 해야한다
 *         => 형식)
 *            [접근지정어] 리턴타입 메소드명() throws 예외처리클래스,...(여러개)
 *            => 처리
 *            메소드() throws 예외처리클래스{
 *               메소드 호출
 *            }
