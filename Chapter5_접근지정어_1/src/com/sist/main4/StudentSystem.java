...
//패키짖명이 다르면 => 사용시에 반드시 import를 사용한다.
import java.util.Scanner;
public class StudentSystem {
	private Student[] std=new Student[3]; // 배열 크기를 만들어준거지 메모리 할당을 할 수 있는 건 아니다.
	// 1. 학생 초기화
	/*
	 * 1. 명시적 초기화 => 객체 생성 시 모든 객체값이 동일
	 * -----------------------------------------
	 * 2. 생성자 이용
	 * 3. 초기화 블록
	 * 4. 메소드 이용
	...
	public void init() {
		...
		for(int i=0;i<std.length;i++) {
			std[i]=new Student(); // 배열은 모든 저장공간이 null(메모리 연결 해야한다)
		    //     -------------- > 메모리 연결하는 부분
	...
	{                    // com.sist.main5에 접근하고 싶음 -> public 써야함
