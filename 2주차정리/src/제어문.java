 ...
 *    ------------ 댓글, 수정, 삭제
 *    - 단일조건문 : 속도가 늦다(독립적인 문장) => 중복적으로 수행해야할 때 주로 사용
 *    형식)
 *        if(조건문) => 조건문 연산자(부정연산자, 비교연산자, 논리연산자)
 *        {
 *            출력내용 -> true 시 수행
 *        }
 ...
 *        true / false를 나눠서 처리
 *        -----------------------
 *        if(조건문)              (조건문: 로그인이 되었다면) // (조건문:아이디중복체크 ->중복)       //(조건문:검색된 내용이 있는 경우)
 *        {
 *            조건이 true일 때 처리 (main으로 이동)             (이미 사용중인 아이디입니다 -> 메세지)   (검색된 내용 출력)
 *        }
 *        else
 *        {
 *            조건이 false일 때 처리 (회원가입으로 이동)          (사용이 가능한 아이디입니다)            (검색된 내용이 없습니다)
 *        }
 *    - 다중조건문 : 조건이 많은 경우 해당되는 조건만 수행이 가능
 *      형식)
 *          if(조건문)
 *          {
 *              조건문이 true 일 때 수행되는 문장 ==> 종료
 *              조건문이 false 일 때 아래 조건문으로 이동
 *          }
 *          else if(조건문)
 *          {
 *              조건문이 true 일 때 수행되는 문장 ==> 종료
 ...
 *          {
 *          } // 이렇게 작성하면 아이디랑 비밀번호가 같지 않을 경우 뭐가 틀렸는지 확인 할 수 없음
 *          웹 -> 일반사용자 -> 최대한 서비스가 좋아야함.
 *          ////////////////// 따라서 아래 코드처럼 작성을 해야 아이디와 비밀번호 중에 뭐가 틀린지 알 수 있음.
 *          if(id,equals(""))
 *          {
 *             if(pwd.equals(""))
 *             {
 ...
 *    - 선택문 : 네트워크, 게임(키값)
 *      형식)
 *         switch(정수,문자,문자열) => 해당위치부터 break; 가 있을 때까지 수행
 *         {
 *            case "Find";
 *            case "Search";
 *            case "Login";
 ...
 *              System.out.println(a);
 *              break;
 *          }  -> 같은 블럭 안에서는 같은 변수명을 사용해서 이 문장은 오류다!
 *   
 * 2. 반복문
 *    ------------ 목록, 페이징(화면출력)
 *    1) for: 반복횟수가 지정된 경우
...
//		}
//			break;
//		}// 같은 변수명을 사용하고 싶으면, 블록 밖에 int 변수명 을 먼저 선언하거나, case별로 블럭을 지정하면 된다.
		
		String[] title= {"Kitsch","CHRISTIAN","꽃","Ditto","Hype boy","Teddy Bear","사건의 지평선","OMG",
				"심 (心)","I Don't Think That I Like Her","사랑은 늘 도망가","건물 사이에 피어난 장미","우리들의 블루스",
				"빙글빙글 (Prod. by R.Tee)","가질 수 없는 너","After LIKE","다시 만날 수 있을까","밤이 무서워요 (Lonely Night)",
				"이제 나만 믿어요","London Boy","파이팅 해야지 (Feat. 이영지)","Polaroid","무지개","Attention","아버지","A bientot",
				"LOVE DIVE","손이 참 곱던 그대","사랑인가 봐","사랑해 진짜","연애편지","보금자리","인생찬가","ANTIFRAGILE",
				"NIGHT DANCER","Dangerously","Zero","The Drum","Like Crazy","바보가슴","Nxde","사랑하기 싫어",
				"새삥 (Prod. by ZICO) (Feat. 호미들)","That's Hilarious","Nostalgia","흔들리는 꽃들 속에서 네 샴푸향이 느껴진거야",
				"Monologue","VIBE (Feat. Jimin of BTS)","Dynamite","오르트구름","그때 그 순간 그대로 (그그그)","다정히 내 이름을 부르면",
		...
		int j=0;
		int rowSize=10;
		int pagecnt=(page*rowSize)-rowSize; // start (30개씩 나누고 싶으면 (a*30)-30
		/*
		 * start    end
		 *   0       9  -> 1page
		 *  10       19 -> 2page
		...
		try
		{
			//FileWriter : 파일 쓰기,
			FileReader fr=
					new FileReader("/Applications/javastudy/2주차정리_2/src/제어문_1.java");
			int i=0;
			while((i=fr.read())!=-1)
			{
