 ...
 *	***= 객체 지향 프로그램(재사용 편리한 상태) => 유지보수
 *			재사용 : 수정, 추가, 데이터 보호(캡슐화) ==> 5장에서
 *				   --------- 오버라이딩/오버로딩
 *				  포함 (수정할 내용이 없는 경우) => has-a
 *				  상속 (수정할 내용이 있는 경우) => is-a
 *		= 비교적 배우기 쉽다
 *		  C/C++에서 파생 => 포인터(메모리 주소), 구조체, 다중상속, 지원하는 API가 미약
 *		  => 자바언어 => 강력한API를 갖고 있다 (필요한 기능이 이미 제작)
 *			=> 조립식 (자바 공부 => 직접 구현)
 *		= 자동 메모리 관리 (가비지 컬렉션)
 ...
 *		6) 객체 지향 프로그램 
 *		7) 프로그램이 비정상 종료를 방지 / 오류를 처리 = 예외처리 (ex, 계산기에서 8/0 할 때 '0으로는 나눌 수 없습니다' 라는 말이 나오게 하기)
 *		============================================= 자바 기본
 *		8) 자바에서 지원하는 라이브러리
 *	3. 자바의 구성
 ...
 *			2a => a2, a3...
 *		3) 특수문자 사용이 가능하다 ( _ , $ )
 *			a_b, _a, file_name, fileName...(임시변수, 단어가 2개인 경우)
 ...
 *		========================================
 *		ex, 과일 가게에는 사과가 5, 배 10, 바나나 7, 딸기 1 ---> 과일종류 4개가 변수 4개!
 *		
 ...
 *			ex, 10.5F // F가 붙으면 float , 없으면 double(원래 D가 붙었는데 생략가능)
 *		  문자 저장 방법
 *			2byte => char
 *			  => 1byte(싱글바이트 => ASC) / 2byte(멀티파이트 => UniCode)
 ...
 *		 4     8
 * 		데이터형 크기
 * 		byte < short , char < int < long < float < double // 숫자 표현이 많아질 수록 커기지 떄문에 실수가 정수보다 크기가 크다.
 * 		
...
//		long a=1000000000000l; // 그냥 숫자만 입력하면 int 데이터로 인식함. 따라서 long이면 숫자 마지막에 'l'을 적어줘야함.(대소문자 상관없음)
//		long a=(long)10.5; // 데이터형 변형
//		int a=12345;
//		double b=123456.78;
//		int c=(int)((b-a)*100);
...
//		// 127을 byte_value라는 공간에 첨부하라 (= 오른쪽에서 왼쪽으로 이동)
//		short short_value=32767; // -32768~32767
//		int int_value=2140000000;
//		long long_vlaue=999999999999999L;
//		// int/long 의 구분은 l(L)
//		char cahr_value='홍';
//		float float_value=10.5f;
//		double double_vlaue=10.5; // 초기값! d(D)는 생략가능
//		// float/duble 의 구분은 f(F) 
...
//		int kor=80;		// 변경할 수 있는 수라서 변수
//		kor=90;
//		kor=100;
//		final int SCORE=70;	// 변경할 수 없는 수라서 상수. 변수명 '대문자'
		/*
		 * 변수
		 * 선언
		 * 초기화
		...
		int a=10;
		int b=20;
		int temp=a; // temp는 임시변수(기억해두고 싶은걸 temp로 기억시켜두자)
		a=b; // a=20 (a를 b로 바꾸면서 a의 마지막 값이 b값이 되어버렸기 때문에 필요한 a값을 temp로 기억시키는것)
		b=temp; // b2=0
		System.out.println("a="+a); //10+"20" => 1020
