 ...
 *    3. 임의 발생 : throw
 *    4. 사용자 정의 : 지원하지 않는 경우에 직접 제작(사용빈두는 거의 없다)
 *       ---------------------------------- if문을 주로 사용한다
 */
import java.awt.*;
import javax.swing.*;
...
public class 예외처리_예외복구_7 extends JFrame implements ActionListener{
	...
	// 버튼 처리하는 메소드 : 정의(용도가 여러개이기 떄문에 구현을 하지 못한다)
	// 호출은 자동 호출 => 구현은 사용자에게 맡긴다
	@Override
	public void actionPerformed(ActionEvent e) {
		...
		if(e.getSource()==b2) { // b2버튼을 클릭했다면
		...
		else if(e.getSource()==b1) { // b1버튼을 클릭했다면
			...
			if(strNum.trim().length()<1) { //조건문 : 좌우 공백이 제거된 상태에서 입력이 안된 경우
