 ...
 *          = 단점) 다른 클래스와 연결이 안됨 => getter/setter를 통해 사용
 *          --> 변수는 은닉화를 하고 메소드를 통해서 접근하는 방식(캡슐화)
 *          --> 기술면접(은닉화 vs 캡슐화)
 *              - 캡슐화 : 데이터를 보호할 목적으로
 * default : 같은 페키지 안에서 접근 가능
...
public class 클래스_접근지정어 {
	 ...
	 * Value v=a;
	 */
	static void swap(Value v) { // 오버로딩(매개변수가 다르까 swap이란 이름을 중복해서 사용 가능)
